...
require __DIR__ . '/admin_credentials.php';
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Check if admin login form submitted
    if (isset($_POST['admin_login'])) {
        $username = trim($_POST['admin_username'] ?? '');
        $password = $_POST['admin_password'] ?? '';

        if ($username === '') $adminErrors[] = 'Username is required.';
        if ($password === '') $adminErrors[] = 'Password is required.';
        ...
        if (!$adminErrors) {
            if ($username === ADMIN_USERNAME && $password === ADMIN_PASSWORD) {
                ...
                $adminErrors[] = 'Invalid admin credentials.';
            }
        }
    } else { 
        // Regular user login
...
<main>
    <section class="card">
        <div class="card-header"><div class="card-title">User Login</div></div>

        <?php if ($message): ?>
            <div class="info-box"><?= htmlspecialchars($message) ?></div>
        <?php endif; ?>
        ...
        <!-- Admin Login -->
        <hr>
        <button id="showAdminLogin">Login as Admin</button>
        <div id="adminLoginForm" style="display:none; margin-top:20px;">
            <?php if ($adminMessage) echo "<div class='info-box'>" . htmlspecialchars($adminMessage) . "</div>"; ?>
            <?php if ($adminErrors): ?>
                <div class="error-box">
                    <ul><?php foreach ($adminErrors as $err): ?><li><?= htmlspecialchars($err) ?></li><?php endforeach; ?></ul>
                </div>
            <?php endif; ?>
